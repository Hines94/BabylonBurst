...
function SetupMaterial(displayerElement: HTMLDivElement, originalData: any, callback: (newData: any) => void): any {
    ...
    const existing = materialDescription.find(
        (val: MateralDescription) => val.constructor.name === originalData["MaterialShaderType"],
    );
    if (existing) {
    ...
    shaderSelect.addEventListener("change", () => {
        ...
        if (option) {
            SetupForShader(
                materialDescription.find((val: MateralDescription) => val.constructor.name === selected),
                displayerElement,
                newData,
            );
