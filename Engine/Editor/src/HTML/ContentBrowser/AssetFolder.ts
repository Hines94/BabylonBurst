...
export class AssetFolder extends VisualItem {
    ...
    GetBundleWithName(name: string) {
        const test = this.containedItems.filter(b => {
            return b.name === name;
        });
        if (test.length > 0) {
            return test[0];
    ...
    GetFolderWithName(name: string) {
        const test = this.containedFolders.filter(b => {
            return b.name === name;
        });
        if (test.length > 0) {
            return test[0];
    ...
    getNewFolderName(): string {
        ...
        var i = 0;
        while (
            this.containedFolders.filter(b => {
                return b.name === newName;
            }).length !== 0
        ) {
            i += 1;
    ...
    async RemoveBundle(bundle: AssetBundle, bSaveOut = true): Promise<boolean> {
        ...
        this.containedItems = this.containedItems.filter(i => {
            return i !== bundle;
        });
        if (bSaveOut) {
    ...
    RemoveFolder(folder: AssetFolder) {
        ...
        this.containedFolders = this.containedFolders.filter(i => {
            return i !== folder;
        });
        return true;
    }
