...
export function SetupContentInputWithDatalist(
    contentType: ContentItemType,
    dropdownSelector: HTMLInputElement,
    onChange: (val: ContentItem) => void,
    message = "Changed data item to",
) {
    ...
    dropdownSelector.addEventListener("change", () => {
        const option = Array.from(datalist.querySelectorAll("option")).find(
            opt => opt.value === dropdownSelector.value,
        );
        if (option) {
