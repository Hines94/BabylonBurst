...
import {
  AsyncAssetLoader,
  GetPreviouslyLoadedAWSAssetCustomPath,
...
export function GetSceneLoader(
  ...
  if (ret === null) {
    ...
    if (gltf === null) {
      console.error(
        "AWS Asset has been loaded incorrectly? Should be a GLTF? " +
          path +
...
export class SceneAsyncLoader extends AsyncAssetLoader {
      ...
      if (LoadedMeshes[i].id.includes(meshName)) {
        ...
        if (asMesh === null || asMesh === undefined) {
          console.error(
            "Mesh " +
              LoadedMeshes[i].name +
              " is not a mesh! Its abstract something..."
          );
        } else {
  ...
  override GetAssetFullPath(): string {
    const fullPath =
      GetAsyncSceneIdentifier(this.desiredScene) + super.GetAssetFullPath();
    return fullPath;
  }
