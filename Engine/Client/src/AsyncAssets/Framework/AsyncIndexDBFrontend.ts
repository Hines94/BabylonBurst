...
export class AsyncIndexDBFrontend implements IFrontendStorageInterface {
  ...
  async AwaitCurrentTransaction(): Promise<boolean> {
    var loader = this;
    if (
      loader.frontendCache.transaction === undefined ||
      loader.frontendCache.transaction === null
    ) {
      return true;
    }
    return await new Promise((resolve, reject) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = function () {
        ...
        if (request.result.objectStoreNames.contains(RequestCacheName)) {
          ...
          console.error(
            "TODO implement database versioning. We need a store name that is not present!!"
          );
        }
      };
      ...
      request.onerror = function (event: any) {
        console.error(
          "Error with setting up Mech Crawler cache database: " +
      ...
      request.onupgradeneeded = function () {
        ...
        loader.frontendCache =
          localDatabase.createObjectStore(RequestCacheName);
        resolve(request.result);
      };
  ...
  async OpenReadTransaction(): Promise<boolean> {
    this.frontendCache = localDatabase
      .transaction(RequestCacheName, "readonly")
      .objectStore(RequestCacheName);
    return true;
  }
