...
export function UpdateAllMeshDefinitions() {
  ...
  for (
    var i = 0;
    i < Math.min(maxLocUpdatesPerFrame, UpdateRequireSMDefinitions.length);
    i++
  ) {
...
export class AsyncStaticMeshDefinition extends BackgroundCacher {
  ...
  static GetAsyncMeshLoader(
    ...
    return asyncMeshLoaders[GetAsyncSceneIdentifier(scene)][
      GetAssetFullPath(desiredPath, fileIndex)
    ];
  }
  ...
  async loadInMesh(scene: Scene) {
    ...
    if (
      AsyncStaticMeshDefinition.GetAsyncMeshLoader(
        scene,
        this.desiredPath,
        this.fileIndex
      ) === undefined
      ...
      asyncMeshLoaders[GetAsyncSceneIdentifier(scene)][
        GetAssetFullPath(this.desiredPath, this.fileIndex)
    ...
    const asyncLoader = AsyncStaticMeshDefinition.GetAsyncMeshLoader(
      scene,
      this.desiredPath,
      this.fileIndex
    );
    ...
    for (var i = 0; i < mats.length; i++) {
      ...
      if (mats[i] !== null) {
        if (
          foundMeshElements[i].material !== null &&
          foundMeshElements[i].material !== undefined
        ) {
    ...
    if (foundMeshElements.length === 1) {
      this.finalCombinedMeshes[GetAsyncSceneIdentifier(scene)] = (
        foundMeshElements[0] as Mesh
      ).clone();
      this.finalCombinedMeshes[
        GetAsyncSceneIdentifier(scene)
      ].makeGeometryUnique();
    } else {
      this.finalCombinedMeshes[GetAsyncSceneIdentifier(scene)] =
        await Mesh.MergeMeshesAsync(
          foundMeshElements,
          true,
          false,
          undefined,
  ...
  setReplacementErrorBox(scene: Scene) {
    this.finalCombinedMeshes[GetAsyncSceneIdentifier(scene)] =
      MeshBuilder.CreateBox("ReplacementErrorBox");
    this.finalCombinedMeshes[GetAsyncSceneIdentifier(scene)].material =
      new StandardMaterial("ReplacementMat");
    this.finalCombinedMeshes[
      GetAsyncSceneIdentifier(scene)
    ].material.alpha = 0.1;
  ...
  protected async populateMeshClone(
    details: StaticMeshCloneDetails
  ): Promise<null> {
  ...
  protected async populateMeshInstance(
    details: StaticMeshInstanceDetails
  ): Promise<null> {
    await this.loadInMesh(details.GetScene());
  ...
  private GetAvailableInstanceIndex(instances: any, sceneID: string): number {
    if (this.availableMeshInstances[sceneID].length > 0) {
      ...
      this.availableMeshInstances[sceneID] = this.availableMeshInstances[
        sceneID
      ].slice(1, this.availableMeshInstances[sceneID].length);
      return val;
    }
  ...
  private colorUpdate(
    mesh: Mesh,
    instances: { [index: number]: StaticMeshInstanceDetails }
  ) {
    if (this.requireColorUpdate === false) {
      return;
    }
    //TODO: This will not work with multiple scenes
    var instanceColors: Float32Array = new Float32Array(
      4 * this.currentInstanceNum
    );
    for (var i = 0; i < this.currentInstanceNum; i++) {
      const offset = i * 4;
      var color = [0, 0, 0, 0];
