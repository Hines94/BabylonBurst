...
export async function FinishClientAutoTypings() {
    ...
    //Auto copy over our client typings to the client for useage
    //@ts-ignore
    let clientInstallPath = process.argv[2];
    //@ts-ignore
    let projectName = process.argv[3];
    if(clientInstallPath) {
        fs.copyFileSync(outputFile, clientInstallPath+`/Autogenerated/${projectName}_ServerTypings_autogenerated.ts`);
    }
